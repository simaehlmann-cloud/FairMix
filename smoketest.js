...
check('Gluecksrad zeichnet je Name ein Segment', () => {
  ...
  g('renderWheel')('draw');
  assert($('wheelBox').hidden === true, 'Rad trotz Namenslauf sichtbar');
...
check('Umschalter auf der Ziehen-Seite wirkt', () => {
  g('chooseDrawStyle')('ticker');
  assert($('switchDrawTicker').classList.contains('choice-active'), 'Namenslauf nicht markiert');
  assert($('wheelBox').hidden === true, 'Rad trotz Namenslauf sichtbar');
  g('chooseDrawStyle')('wheel');
  assert($('switchDrawWheel').classList.contains('choice-active'), 'Glücksrad nicht markiert');
  assert($('choiceDrawWheel').classList.contains('choice-active'), 'Einstellungsseite nicht mitgezogen');
  assert($('wheelBox').hidden === false, 'Rad bleibt versteckt');
});
...
check('Alte gespeicherte Liste ohne Partnerhistorie laedt sauber', () => {
  const saved = JSON.parse(store['fairmix_saved_lists']);
  ...
  $('savedNamesSelect').value = 'alt';
  g('loadNames')();
  assert(g('originalNames').length === 2, 'alte Liste nicht geladen');
...
check('Zu Beginn ist nur der Moderator als Rolle aktiv', () => {
  vm.runInContext("activeRoleIds = [...DEFAULT_ACTIVE_ROLE_IDS];", ctx);
  assert(g('activeRoleIds').length === 1, 'erwartet 1 Rolle, ist ' + g('activeRoleIds').length);
  const aktiv = g('getActiveRoles')();
  assert(aktiv.length === 1, 'getActiveRoles liefert ' + aktiv.length);
  assert(aktiv[0].id === 'r1', 'aktive Rolle ist nicht der Moderator: ' + aktiv[0].id);
  assert(g('getAllRoles')().length >= 6, 'die uebrigen Rollen fehlen ganz');
});

check('Mit einer Rolle bekommt nur eine Person je Gruppe eine', () => {
  ladeDemo();
  vm.runInContext("features.roles = true; activeRoleIds = [...DEFAULT_ACTIVE_ROLE_IDS];", ctx);
  $('teamCount').value = '4'; $('teamSize').value = '';
  g('generateTeams')();
  g('teams').forEach((tm, i) => {
    const mitRolle = tm.filter(m => m.roleId).length;
    assert(mitRolle === 1, 'Gruppe ' + (i + 1) + ' hat ' + mitRolle + ' Rollen');
  });
});
...
check('Praesentation zeigt das Rollensymbol ohne Namen', () => {
  ...
  suche($('presentGrid'));
  assert(gefunden === 4, 'erwartet 4 Rollensymbole, sind ' + gefunden);
  g('closePresentation')();
});

...
check('Gluecksrad zeichnet je Name ein Segment', () => {
  ...
  /* Eigener Ausgangszustand: ohne Zaehlerstand ist der ganze Kurs im Topf. */
  vm.runInContext("drawStyle = 'wheel'; drawCounts = {}; fairDraw = true; wheelState.draw.frozen = false;", ctx);
  g('renderWheel')('draw');
  assert($('wheelBox').hidden === false, 'Rad bleibt versteckt');
  g('renderWheel')('draw');
  const pfade = $('wheelRotor').children.filter(c => c.tagName === 'PATH');
  assert(pfade.length === g('drawPool')().length,
         'Segmentzahl ' + pfade.length + ', Topf ' + g('drawPool')().length);
...
check('Alle Radsegmente sind gleich gross', () => {
  vm.runInContext("drawCounts = {}; fairDraw = true; drawStyle = 'wheel'; wheelState.draw.frozen = false;", ctx);
  g('renderWheel')('draw');
  const spanne = g('wheelState').draw.segs.map(x => x.end - x.start);
  assert(spanne.length > 1, 'zu wenige Segmente: ' + spanne.length);
  assert(Math.max(...spanne) - Math.min(...spanne) < 0.001,
         'Segmente ungleich gross: ' + (Math.max(...spanne) - Math.min(...spanne)));
});

check('Rad bleibt nach der Drehung stehen', () => {
  vm.runInContext("drawCounts = {}; fairDraw = true; wheelState.draw.frozen = false;", ctx);
  g('renderWheel')('draw');
  ...
  /* Der Topf schrumpft ueber den Zaehler, nicht ueber eine zweite Liste. */
  vm.runInContext("drawCounts[rosterNames()[0]] = 1;", ctx);
  g('renderWheel')('draw');
...
check('Klassenleiste erscheint erst ab zwei Klassen', () => {
  /* Eigener Ausgangszustand: fruehere Ablaeufe hinterlassen Klassen. */
  const gemerkt = store['fairmix_saved_lists'];
  store['fairmix_saved_lists'] = JSON.stringify({ '5a': JSON.parse(gemerkt)['5a'] });
  g('renderAll')();
  assert($('classBar').hidden === true, 'Leiste schon bei einer Klasse sichtbar');

  store['fairmix_saved_lists'] = JSON.stringify({});
  g('renderAll')();
  assert($('classBar').hidden === true, 'Leiste ohne Klassen sichtbar');
  store['fairmix_saved_lists'] = gemerkt;
});

check('Klassenleiste folgt ihrem Schalter', () => {
  vm.runInContext("features.classes = false;", ctx);
  g('renderAll')();
  assert($('classBar').hidden === true, 'Leiste trotz abgeschaltetem Bereich sichtbar');
  assert($('saveLoadBox').hidden === true, 'Speicherbereich trotz Abschaltung sichtbar');
  vm.runInContext("features.classes = true;", ctx);
  g('renderAll')();
  assert($('saveLoadBox').hidden === false, 'Speicherbereich bleibt versteckt');
});
...
check('Auswahlliste zeigt alle Klassen, die aktive markiert', () => {
  g('renderAll')();
  assert($('classBar').hidden === false, 'Leiste bei zwei Klassen versteckt');
  const werte = [...$('classSelect').children].map(o => o.value);
  assert(werte.indexOf('5a') >= 0 && werte.indexOf('7b') >= 0,
         'Klassen fehlen: ' + werte.join(', '));
  const markiert = [...$('classSelect').children].filter(o => o.selected).map(o => o.value);
  assert(markiert.length === 1 && markiert[0] === g('currentClass'),
         'falsche Markierung: ' + markiert.join(', ') + ' statt ' + g('currentClass'));
  assert($('classCount').textContent.indexOf('24') === 0,
         'Namenszahl fehlt: ' + $('classCount').textContent);
});
...
check('Ziehen entfernt keinen Namen aus dem Gruppentopf', () => {
  ladeDemo();
  vm.runInContext("drawCounts = {}; drawStyle = 'ticker'; fairDraw = true;", ctx);
  const vorher = g('presentNames').length;
  vm.runInContext("commitDraw(rosterNames()[0], getDisplay() || {classList:{add(){},remove(){}}});", ctx);
  assert(Object.keys(g('drawCounts')).length === 1, 'Ziehung nicht gezaehlt');
  assert(g('presentNames').length === vorher, 'Gruppentopf geschrumpft: ' + g('presentNames').length);
  assert(g('drawPool')().length === vorher - 1, 'Ziehungstopf nicht geschrumpft');
});

check('Gezogene Namen landen weiterhin in Zufallsgruppen', () => {
  const gezogen = Object.keys(g('drawCounts'))[0];
  $('teamCount').value = '4'; $('teamSize').value = '';
  g('generateTeams')();
...
check('Ziehungsstatus zaehlt gegen alle Anwesenden', () => {
  ladeDemo();
  vm.runInContext("drawCounts = {}; fairDraw = true;", ctx);
...
check('Namensliste zeigt Gruppe und Ziehung gleichzeitig', () => {
  ...
  $('groupSelect').value = 'Tisch B';
  g('assignToSelectedGroup')(name);
  vm.runInContext("drawCounts = {}; drawCounts['" + name.replace(/'/g, "\\'") + "'] = 1;", ctx);
  g('renderNameList')();
...
check('Kein Rest des Ablaufmodus im Code', () => {
  ['sequenceOn', 'sequenceOrder', 'sequenceIndex', 'sequenceTimer',
   'toggleSequence', 'applySequence', 'shuffleSequence', 'sequenceStep',
   'seqStatus', 'seqPrev', 'seqNext', 'seqTimerBtn', 'sequence-bar',
   'tipSequence', 'btnSeqNext', 'onWord', 'offWord'].forEach(name => {
    assert(html.indexOf(name) < 0, 'Rest gefunden: ' + name);
  });
});

check('Steuerleiste enthaelt genau die sechs vorgesehenen Schaltflaechen', () => {
  ladeDemo();
  $('teamCount').value = '4'; $('teamSize').value = '';
  g('generateTeams')();
  g('openPresentation')('auto');
  const leiste = documentStub.querySelector('.present-controls');
  assert(leiste, 'Steuerleiste fehlt');
  const knoepfe = leiste.children.filter(k => k.tagName === 'BUTTON');
  const erwartet = ['presentDraw()', 'presentResetDraw()', 'toggleTimerPanel()',
                    'scalePresentation(-1)', 'scalePresentation(1)', 'closePresentation()'];
  assert(knoepfe.length === erwartet.length,
         'erwartet ' + erwartet.length + ' Schaltflaechen, sind ' + knoepfe.length);
  knoepfe.forEach((k, i) => {
    const auf = k.getAttribute('onclick') || '';
    assert(auf === erwartet[i], 'Platz ' + (i + 1) + ': "' + auf + '" statt "' + erwartet[i] + '"');
  });
});

check('Alle Gruppen sind gleichzeitig sichtbar', () => {
  assert($('presentGrid').className.indexOf('sequence') < 0, 'Gitter im Ablaufmodus');
  assert(g('presentCards').length === 4, 'erwartet 4 Karten, sind ' + g('presentCards').length);
  g('presentCards').forEach((c, i) => {
    assert(!c.node.classList.contains('seq-current'), 'Karte ' + i + ' noch markiert');
  });
});

check('Timer bleibt eigenstaendig bedienbar', () => {
  g('setTimerMinutes')(3);
  g('toggleTimerPanel')();
  assert($('timerBox').hidden === false, 'Timer laesst sich nicht einblenden');
  g('startTimer')();
  assert(g('timerRunning') === true, 'Timer startet nicht');
  g('closePresentation')();
  assert(g('timerRunning') === false, 'Timer laeuft nach dem Schliessen weiter');
});


/* ============ Startseite und Icons ============ */

check('Titelbanner traegt das App-Icon', () => {
  const bar = $('fairmixBar');
  assert(bar, 'Banner fehlt');
  const bild = bar.children.filter(k => k.tagName === 'IMG')[0];
  assert(bild, 'kein Icon im Banner');
  assert(bild.getAttribute('src') === 'icon-192.png', 'falsche Quelle: ' + bild.getAttribute('src'));
  /* Rein schmueckend: darf Screenreadern nicht vorgelesen werden. */
  assert(bild.getAttribute('alt') === '', 'Icon ohne leeres alt-Attribut');
  assert(bild.getAttribute('width') && bild.getAttribute('height'),
         'Masse fehlen – das Banner springt beim Laden');

  /* Das Icon traegt Bildinhalt, der erkennbar sein muss: unter 64 Pixel
     verschwimmen die vier Gruppen. */
  const regel = html.match(/#fairmixBar img \{[^}]*\}/)[0];
  const untergrenze = regel.match(/width:\s*clamp\((\d+)px/);
  assert(untergrenze && parseInt(untergrenze[1], 10) >= 64,
         'Icon zu klein: ' + (untergrenze ? untergrenze[1] : '?') + ' px');
});

check('Rot bleibt gefaehrlichen Aktionen vorbehalten', () => {
  /* "Ueber FairMix" oeffnet nur eine Infoseite und war rot wie "Alle loeschen". */
  const start = html.indexOf('id="pageStart"');
  const ende  = html.indexOf('id="pageSettings"');
  const abschnitt = html.slice(start, ende);
  const roteKnoepfe = abschnitt.match(/btn red[^>]*>([^<]*)</g) || [];
  assert(roteKnoepfe.length === 0,
         'rote Schaltflaeche auf der Startseite: ' + roteKnoepfe.join(', '));
});

check('Manifest und Icons stimmen in den Massen ueberein', () => {
  const manifest = JSON.parse(fs.readFileSync('manifest.json', 'utf8'));
  manifest.icons.forEach(eintrag => {
    const soll = parseInt(eintrag.sizes.split('x')[0], 10);
    const daten = fs.readFileSync(eintrag.src);
    /* Breite und Hoehe stehen im IHDR-Block eines PNG ab Byte 16. */
    const breite = daten.readUInt32BE(16);
    const hoehe  = daten.readUInt32BE(20);
    assert(breite === soll && hoehe === soll,
           eintrag.src + ': ' + breite + 'x' + hoehe + ' statt ' + soll + 'x' + soll);
  });
});


/* ============ Ziehen: ein Zaehler, eine Schaltflaeche ============ */

check('Der Topf ergibt sich allein aus dem Zaehler', () => {
  ladeKlasse27();
  vm.runInContext("fairDraw = true; drawStyle = 'ticker';", ctx);
  assert(g('drawPool')().length === 27, 'Start: ' + g('drawPool')().length);
  zieheOft(4);
  assert(g('drawPool')().length === 23, 'nach 4 Ziehungen: ' + g('drawPool')().length);
  g('updateDrawStatus')();
  assert($('drawStatus').textContent.indexOf('23 / 27') === 0, $('drawStatus').textContent);
});

check('Anzeige und Auswahl koennen sich nicht widersprechen', () => {
  /* Der frueher gemeldete Fehler: Der Topf zeigte wieder 27/27, es kamen
     aber weiter nur die dran, die noch nicht gezogen waren. */
  const offen = g('drawPool')().slice();
  const gezogen = [];
  for (let i = 0; i < 23; i++) {
    const c = vm.runInContext("chooseNextName()", ctx);
    gezogen.push(c);
    vm.runInContext("commitDraw(" + JSON.stringify(c) + ", { classList: { add() {}, remove() {} } });", ctx);
  }
  assert(gezogen.length === new Set(gezogen).size, 'jemand kam zweimal dran');
  offen.forEach(n => assert(gezogen.indexOf(n) >= 0, n + ' wurde uebersprungen'));
});

check('Nach einem Durchlauf sind alle wieder im Topf', () => {
  assert(g('drawPool')().length === 27, 'Topf: ' + g('drawPool')().length);
  const werte = g('originalNames').map(n => g('drawCounts')[n] || 0);
  assert(Math.min(...werte) === 1 && Math.max(...werte) === 1, 'Zaehler ungleich');
});

check('Markierung folgt dem Topf', () => {
  ladeKlasse27();
  vm.runInContext("fairDraw = true;", ctx);
  const erster = g('rosterNames')()[0];
  assert(g('bereitsGezogen')(erster) === false, 'vor der Ziehung markiert');
  zieheOft(1);
  const dran = Object.keys(g('drawCounts'))[0];
  assert(g('bereitsGezogen')(dran) === true, 'nach der Ziehung nicht markiert');
  zieheOft(26);
  assert(g('bereitsGezogen')(dran) === false, 'Markierung bleibt nach dem Durchlauf haengen');
});

check('Neue Runde stellt alle gleich', () => {
  ladeKlasse27();
  vm.runInContext("undoStack = []; fairDraw = true;", ctx);
  zieheOft(5);
  g('resetDraw')();
  assert(Object.keys(g('drawCounts')).length === 0, 'Zaehler nicht geleert');
  assert(g('drawPool')().length === 27, 'Topf: ' + g('drawPool')().length);
  g('performUndo')();
  assert(Object.keys(g('drawCounts')).length === 5, 'nicht rueckgaengig gemacht');
});

check('Neue Runde meldet sich, wenn nichts zu tun ist', () => {
  ladeKlasse27();
  vm.runInContext("undoStack = [];", ctx);
  $('msgText').textContent = '';
  g('resetDraw')();
  assert(g('canUndo')() === false, 'leerer Aufruf legt einen Schnappschuss an');
  assert($('msgText').textContent === g('t')('msgNoDrawYet'), 'falsche Meldung');
});

check('Es gibt nur noch eine Schaltflaeche zum Zuruecksetzen', () => {
  assert(html.indexOf('resetDrawHistory') < 0, 'zweite Schaltflaeche noch vorhanden');
  assert(html.indexOf('btnResetHistory') < 0, 'Text der zweiten Schaltflaeche noch vorhanden');
  assert(html.indexOf('wheelWeighted') < 0, 'wirkungslose Gewichtung noch vorhanden');
});

check('Ohne faire Ziehung ist jeder im Topf', () => {
  ladeKlasse27();
  vm.runInContext("fairDraw = false;", ctx);
  zieheOft(5);
  assert(g('drawPool')().length === 27, 'Topf schrumpft trotz reiner Zufallsziehung');
  const irgendwer = g('rosterNames')()[0];
  assert(g('bereitsGezogen')(irgendwer) === false, 'Markierung ohne faire Ziehung');
  vm.runInContext("fairDraw = true;", ctx);
});

check('Abwesende zaehlen beim Mindestwert nicht mit', () => {
  ladeKlasse27();
  vm.runInContext("fairDraw = true;", ctx);
  zieheOft(27);                       /* alle auf 1 */
  const weg = g('rosterNames')()[0];
  vm.runInContext("drawCounts[" + JSON.stringify(weg) + "] = 0;", ctx);
  g('markAbsent')(weg);
  /* Der Abwesende haette den Mindestwert auf 0 gedrueckt und alle anderen
     faelschlich als "schon gezogen" markiert. */
  assert(g('drawPool')().length === 26, 'Topf: ' + g('drawPool')().length);
});


/* ============ Klassenwechsel direkt aus der Auswahl ============ */

check('Auswahl wechselt die Klasse ohne zweiten Tipp', () => {
  ladeDemo();
  $('saveNameInput').value = 'Klasse Eins';
  g('saveNames')();
  vm.runInContext("originalNames = ['Nur A','Nur B']; presentNames = [...originalNames]; absentNames = []; groups = {}; teams = []; pairHistory = {}; drawCounts = {}; levels = {}; pairRules = {together:[],apart:[]};", ctx);
  $('saveNameInput').value = 'Klasse Zwei';
  g('saveNames')();
  g('renderAll')();

  $('classSelect').value = 'Klasse Eins';
  g('switchToSelectedClass')();
  assert(g('currentClass') === 'Klasse Eins', 'nicht gewechselt: ' + g('currentClass'));
  assert(g('originalNames').length === 24, 'Namen nicht geladen');
});

check('Auswahl der aktiven Klasse laedt nicht neu', () => {
  /* Ein Neuladen derselben Klasse waere sichtbar: es legt einen
     Schnappschuss an und wuerfe ungespeicherte Aenderungen weg. */
  vm.runInContext("undoStack = [];", ctx);
  $('nameInput').value = 'Frisch Dazu';
  g('addName')();
  const vorher = g('originalNames').length;
  ...
  assert(g('currentClass') === 'Klasse Eins', 'Klasse gewechselt');
  assert(g('originalNames').length === vorher,
         'Klasse wurde neu geladen, Aenderung verloren: ' + g('originalNames').length);
  assert(g('originalNames').indexOf('Frisch Dazu') >= 0, 'neuer Name verschwunden');
  assert(g('canUndo')() === false, 'ueberfluessiger Schnappschuss angelegt');
});

check('Nach dem Wechsel zeigt das Feld die geladene Klasse', () => {
  $('classSelect').value = 'Klasse Zwei';
  g('switchToSelectedClass')();
  const markiert = [...$('classSelect').children].filter(o => o.selected).map(o => o.value);
  assert(markiert[0] === 'Klasse Zwei', 'Feld zeigt ' + markiert[0] + ' statt der geladenen Klasse');
  assert($('classCount').textContent.indexOf('2') === 0, 'Namenszahl falsch: ' + $('classCount').textContent);
});

check('Kein Wechseln-Knopf mehr noetig', () => {
  assert(html.indexOf('classSwitchBtn') < 0, 'Wechseln-Knopf noch vorhanden');
  assert(html.indexOf('btnSwitchClass') < 0, 'Text des Wechseln-Knopfs noch vorhanden');
  const bar = html.match(/<div id="classBar"[\s\S]*?<\/div>/)[0];
  assert(bar.indexOf('onchange="switchToSelectedClass()"') >= 0,
         'Auswahl loest keinen Wechsel aus');
});

check('Klassenleiste ist eine Zeile, kein Kasten', () => {
  const bar = html.match(/<div id="classBar"[^>]*>/)[0];
  assert(bar.indexOf('settings-box') < 0, 'Leiste ist noch ein Kasten');
  assert(bar.indexOf('class-bar') >= 0, 'Zeilen-Klasse fehlt');
  const regel = html.match(/\.class-bar \{[^}]*\}/)[0];
  assert(regel.indexOf('flex') >= 0, 'Leiste ist nicht einzeilig gesetzt');
});


/* ============ Ergebnisse der Code-Pruefung ============ */

check('Backup: unsinnige Formatversionen werden abgewiesen', () => {
  const p = g('backupProblem');
  const rumpf = { app: 'FairMix', state: { originalNames: ['A'] } };
  /* parseInt(1e308) lieferte 1 und liess die Datei durch. */
  assert(p(Object.assign({ schema: 1e308 }, rumpf)) === 'msgBackupInvalid', '1e308 angenommen');
  assert(p(Object.assign({ schema: -1 }, rumpf)) === 'msgBackupInvalid', 'negativ angenommen');
  assert(p(Object.assign({ schema: 1.5 }, rumpf)) === 'msgBackupInvalid', 'Bruchzahl angenommen');
  assert(p(Object.assign({ schema: '1' }, rumpf)) === null, 'Ziffernfolge abgewiesen');
  assert(p(Object.assign({ schema: 1 }, rumpf)) === null, 'gueltiges Backup abgewiesen');
});

check('Backup: kaputte Klasseneintraege werden abgewiesen', () => {
  const p = g('backupProblem');
  const rumpf = { app: 'FairMix', schema: 1, state: { originalNames: ['A'] } };
  assert(p(Object.assign({ savedLists: { x: null } }, rumpf)) === 'msgBackupInvalid', 'null-Eintrag angenommen');
  assert(p(Object.assign({ savedLists: { x: {} } }, rumpf)) === 'msgBackupInvalid', 'Eintrag ohne Namen angenommen');
  assert(p(Object.assign({ savedLists: { x: [] } }, rumpf)) === 'msgBackupInvalid', 'Array-Eintrag angenommen');
  assert(p(Object.assign({ savedLists: { x: { original: ['A'] } } }, rumpf)) === null, 'gueltige Klasse abgewiesen');
  assert(p(Object.assign({ savedLists: {} }, rumpf)) === null, 'leere Klassenliste abgewiesen');
});

check('Import laesst bei Fehlschlag keinen Mischzustand zurueck', async () => {
  ladeDemo();
  $('saveNameInput').value = 'Alt';
  g('saveNames')();
  const zustandVorher = store['fairmix_full_state'];
  const listenVorher  = store['fairmix_saved_lists'];
  assert(zustandVorher && listenVorher, 'Testvorbereitung fehlgeschlagen');

  /* Der zweite Schreibvorgang scheitert – wie bei vollem Speicher. */
  const echteBeschreibung = Object.getOwnPropertyDescriptor(store, 'fairmix_saved_lists');
  let schreibversuche = 0;
  Object.defineProperty(store, 'fairmix_saved_lists', {
    configurable: true,
    get() { return listenVorher; },
    set() { schreibversuche++; throw new Error('QuotaExceededError'); }
  });

  try {
    g('importDataFromObject')({
      app: 'FairMix', schema: 1,
      state: { originalNames: ['Fremd A', 'Fremd B'], presentNames: ['Fremd A'], absentNames: [] },
      savedLists: { Fremd: { original: ['Fremd A'] } }
    });
  } finally {
    delete store['fairmix_saved_lists'];
    if (echteBeschreibung) Object.defineProperty(store, 'fairmix_saved_lists', echteBeschreibung);
    else store['fairmix_saved_lists'] = listenVorher;
  }

  assert(schreibversuche > 0, 'zweiter Schreibvorgang fand nicht statt');
  assert(store['fairmix_full_state'] === zustandVorher,
         'Zustand wurde nicht zurueckgerollt – fremde Daten neben alten Klassen');
});

check('Gruppenzahl: unsinnige Eingaben melden sich', () => {
  ladeDemo();
  $('teamSize').value = '';
  [['-5', 'negativ'], ['2.7', 'Bruchzahl'], ['abc', 'Text'], ['1e9', 'Exponent']].forEach(([wert, was]) => {
    $('teamCount').value = wert;
    $('msgText').textContent = '';
    vm.runInContext("teams = [];", ctx);
    g('generateTeams')();
    if (was === 'Exponent') return;   /* 1e9 ist ganzzahlig und wird gedeckelt */
    assert(g('teams').length === 0, was + ' erzeugte Gruppen');
    assert($('msgText').textContent === g('t')('msgEnterNumber'),
           was + ': keine Rueckmeldung, sondern "' + $('msgText').textContent + '"');
  });
  $('teamCount').value = '4';
});

check('Sehr grosse Gruppenzahl wird gedeckelt', () => {
  ladeDemo();
  $('teamCount').value = '1000000000'; $('teamSize').value = '';
  g('generateTeams')();
  assert(g('teams').length === 24, 'erwartet 24 Einzelgruppen, sind ' + g('teams').length);
});

check('Partnerhistorie waechst nicht unbegrenzt', () => {
  const namen = [];
  for (let i = 0; i < 300; i++) namen.push('K' + i);
  vm.runInContext("originalNames = " + JSON.stringify(namen) + "; presentNames = [...originalNames]; absentNames = []; groups = {}; teams = []; pairHistory = {}; drawCounts = {}; features.partners = true; features.rules = false; features.levels = false; levelMode = 'off';", ctx);
  $('teamCount').value = '50'; $('teamSize').value = '';
  for (let i = 0; i < 15; i++) g('generateTeams')();

  const anzahl = Object.keys(g('pairHistory')).length;
  assert(anzahl <= g('PAIR_HISTORY_MAX'),
         'Historie ueber der Grenze: ' + anzahl + ' von hoechstens ' + g('PAIR_HISTORY_MAX'));
  assert(anzahl > 1000, 'Historie unerwartet klein: ' + anzahl);

  /* Beim Kappen muss nach Haeufigkeit sortiert werden: der kleinste
     behaltene Wert darf nicht unter dem groessten weggeworfenen liegen. */
  const werte = Object.values(g('pairHistory'));
  assert(Math.min(...werte) >= 1, 'Nullwerte in der Historie');
});

check('Beim Kappen ueberleben die haeufigsten Paare', () => {
  const max = g('PAIR_HISTORY_MAX');
  const viel = {};
  for (let i = 0; i < max + 500; i++) viel['A' + i + '\u001FB' + i] = 1;
  viel['Wichtig\u001FPaar'] = 99;
  vm.runInContext("pairHistory = " + JSON.stringify(viel) + "; trimPairHistory();", ctx);
  assert(Object.keys(g('pairHistory')).length === max, 'nicht gekappt: ' + Object.keys(g('pairHistory')).length);
  assert(g('pairHistory')['Wichtig\u001FPaar'] === 99, 'haeufigstes Paar weggeworfen');
});

check('Gekappte Historie bleibt im Speicherrahmen', () => {
  const groesse = JSON.stringify(g('pairHistory')).length;
  assert(groesse < 200000, 'Historie belegt ' + Math.round(groesse / 1024) + ' KB');
});

check('Alte uebergrosse Sicherung wird beim Laden gekappt', () => {
  /* Namen und Historie muessen zueinander passen, sonst filtert
     loadState() ohnehin alles weg und der Deckel bliebe ungeprueft. */
  const namen = [];
  const viel = {};
  const max = g('PAIR_HISTORY_MAX');
  for (let i = 0; i < 200; i++) namen.push('P' + i);
  let gesetzt = 0;
  for (let a = 0; a < 200 && gesetzt < max + 1500; a++) {
    for (let b = a + 1; b < 200 && gesetzt < max + 1500; b++) {
      viel['P' + a + '\u001FP' + b] = 1; gesetzt++;
    }
  }
  assert(gesetzt > max, 'Testvorbereitung: nur ' + gesetzt + ' Paare');

  store['fairmix_full_state'] = JSON.stringify({
    originalNames: namen, presentNames: namen, absentNames: [], pairHistory: viel
  });
  vm.runInContext('loadState();', ctx);
  const nachher = Object.keys(g('pairHistory')).length;
  assert(nachher <= max, 'nach dem Laden nicht gekappt: ' + nachher);
  assert(nachher > 0, 'Historie komplett verloren');
});

check('Keine Netzwerkzugriffe und kein dynamisches JavaScript', () => {
  const js = html.slice(html.indexOf('<script'), html.lastIndexOf('</script>'));
  [/\bfetch\s*\(/, /XMLHttpRequest/, /WebSocket/, /\beval\s*\(/, /new\s+Function\s*\(/,
   /document\.write/].forEach(muster => {
    assert(!muster.test(js), 'gefunden: ' + muster);
  });
});

check('Nutzertexte gehen nie durch innerHTML', () => {
  /* Alle innerHTML-Zuweisungen duerfen nur zum Leeren dienen. */
  const treffer = html.match(/innerHTML\s*=\s*[^;]+/g) || [];
  treffer.forEach(t => {
    assert(/innerHTML\s*=\s*(''|"")/.test(t), 'innerHTML mit Inhalt: ' + t);
  });
});

check('Speicher nutzt nur die vier bekannten Schluessel', () => {
  const schluessel = [...new Set((html.match(/localStorage\.(?:get|set|remove)Item\("([a-z_]+)"/g) || [])
    .map(x => x.match(/"([a-z_]+)"/)[1]))].sort();
  assert(schluessel.join(',') === 'fairmix_full_state,fairmix_lang,fairmix_saved_lists,fairmix_theme',

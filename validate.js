...
/* ---------- 7b. Glücksrad: Drehpunkt vollständig definiert ---------- */
const rotoren = [...html.matchAll(/<g id="(wheelRotor[A-Za-z]*)"([^>]*)>/g)];
if (!rotoren.length) E('Keine Rad-Gruppen im Markup gefunden');
const ohneKlasse = rotoren.filter(m => !/class="[^"]*\bwheel-rotor\b/.test(m[2])).map(m => m[1]);
if (ohneKlasse.length) E(`Rad-Gruppen ohne Klasse wheel-rotor: ${ohneKlasse.join(', ')}`);
if (!/\.wheel-rotor\s*\{[^}]*transform-box:\s*view-box/.test(html))
  E('.wheel-rotor ohne transform-box: view-box – die Scheibe verrutscht nach der Drehung');
if (!/\.wheel-rotor\s*\{[^}]*transform-origin:\s*100px\s+100px/.test(html))
  E('.wheel-rotor ohne transform-origin: 100px 100px');
const raeder = [...js.matchAll(/rotor:\s*"([^"]+)"/g)].map(m => m[1]);
const fehlend = raeder.filter(r => !rotoren.some(m => m[1] === r));
if (fehlend.length) E(`WHEELS verweist auf fehlende Rad-Gruppen: ${fehlend.join(', ')}`);
if (!errors.some(e => /Rad-Gruppen|wheel-rotor/.test(e)))
  O(`Alle ${rotoren.length} Glücksräder mit korrektem Drehpunkt`);
...
for (const [re, msg] of [
  ...
  [/Orientation"\s+value="(default|sensor|landscape)"/,
